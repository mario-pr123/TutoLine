...
class TutoriasController extends Controller
{
    ...
    public function index()
    {
        ...
        ->select([DB::raw("concat(tutors.nombre_tutor,' ',tutors.apellido_tutor) as namesTutor"),//SQL
        DB::raw('group_concat(concat(alumnos.nombre_alumno," ",alumnos.apellido_alumno) SEPARATOR ", ") as names'),
        'alumno_tutors.id','alumno_tutors.tutor_id_tutor'
    ...
    public function eliminar()
    {
        ...
        $combo = Alumno::all();
        $combo1 = Tutor::all();
        return view('tutorias.eliminar')->with('alumnos',$combo)->with('tutors',$combo1)->with('tutorias',$elementos);
    }
    ...
    public function verAlumnos(Request $request)
    {
        ...
        $combo =$combo->where('alumno_tutors.tutor_id_tutor',$request->tutor_id_tutor)
            ...
            ->select(['alumnos.nombre_alumno','alumnos.apellido_alumno',DB::raw("sum(tareas.calificacion_tarea) as nota_total"),
            DB::raw("sum(tareas.puntaje_extra) as extra_total"),DB::raw("count(*) as tarea_total")
            ,'alumno_tutors.alumno_id_alumno'])
            ->get();
